    ...
    private Map<ActualTableName, List<IntrospectedColumn>> getColumns(
        ...
        while (rs.next()) {
			...
			PreparedStatement localPreparedStatement = connection
					.prepareStatement("select * from user_col_comments  where table_name='" + localTableName
							+ "' and COLUMN_NAME='" + introspectedColumn.getActualColumnName() + "'");
			ResultSet localOracleResultSet = localPreparedStatement.executeQuery();
			localOracleResultSet.next();
			introspectedColumn.setRemarks(localOracleResultSet.getString("COMMENTS"));
			// oracle字段 注释结束
        ...
        try {
            ...
            if (rs.next()) {
				...
				PreparedStatement localPreparedStatement = connection.prepareStatement(
						"select * from user_tab_comments  where table_name='" + fqt.getIntrospectedTableName() + "'");
    			ResultSet localOracleResultSet = localPreparedStatement.executeQuery();
    			localOracleResultSet.next();
    			introspectedTable.setRemarks(localOracleResultSet.getString("COMMENTS"));
    			// oracle表 注释结束
